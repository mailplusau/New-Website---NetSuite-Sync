...
function addContact(request, response) {
	if (request.getMethod() == "GET") {
		...
		var activeCustomerListSignedSearch = nlapiLoadSearch(
			"customer",
			"customsearch_active_customers_2"
		);
		...
		var activeCustomerListSignedSearchResult =
			activeCustomerListSignedSearch.runSearch();

		var activeCustomerListSignedSearchResultSet =
			activeCustomerListSignedSearchResult.getResults(0, 1);
		...
		if (activeCustomerListSignedSearchResultSet.length != 0) {
			...
			activeCustomerListSignedSearchResult.forEachResult(function (
				searchResultActiveCustomerListSigned
			) {
				custInternalID =
					searchResultActiveCustomerListSigned.getValue("internalid");
				custEntityID =
					searchResultActiveCustomerListSigned.getValue("entityid");
				...
				intitial_customer_status =
					searchResultActiveCustomerListSigned.getValue("entitystatus");
				partner_text = searchResultActiveCustomerListSigned.getText("partner");
				accountManager = searchResultActiveCustomerListSigned.getText(
					"custentity_mp_toll_salesrep"
				);
				accountManagerId = searchResultActiveCustomerListSigned.getValue(
					"custentity_mp_toll_salesrep"
				);

				var franchiseeSalesRepAssigned = nlapiLookupField(
					"customer",
					parseInt(custInternalID),
			...
			try {
				...
				contactRecordNew.setFieldValue("custentity_connect_admin", 1);
				contactRecordNew.setFieldValue("custentity_connect_user", 1);
				contactRecordNew.setFieldValue(
					"entityid",
			...
			nlapiSendEmail(
				salesRepId,
				email,
				custEntityID + " " + custName + " - " + templateSubject,
				emailHtml,
				["portalsupport@mailplus.com.au"],
				[
			...
			email_body += "Customer Name: " + custEntityID + " " + custName + "</br>";
			email_body += "Franchisee: " + partner_text + "</br></br>";

			var email_subject =
				"MP Portal - Link User to Customer - " + custEntityID + " " + custName;
			...
			if (intitial_customer_status == 50 || intitial_customer_status == 72) {
				...
				var commRegUpdateTnCAgreedDateSearch = nlapiLoadSearch(
					"customrecord_commencement_register",
					"customsearch_comm_reg_upd_tnc_date"
				);

				var filCommReg = [];
				filCommReg[filCommReg.length] = new nlobjSearchFilter(
					"internalid",
					"custrecord_customer",
					"anyof",
					custInternalID
				...
				var commRegUpdateTnCAgreedDateSearchResult =
					commRegUpdateTnCAgreedDateSearch.runSearch();

				commRegUpdateTnCAgreedDateSearchResult.forEachResult(function (
					searchResult
				) {
				...
				nlapiRequestURL(
					"https://mpns.protechly.com/new_customer",
					customerJSON,
					headers
				);
				...
				emailBody = emailBody.replace(/nlemaccountmanager/gi, salesRepName);
				var emailAttach = new Object();
				emailAttach["entity"] = custInternalID;
				...
				emailBody2 = emailBody2.replace(/nlemfirstname/gi, first_name);
				emailBody2 = emailBody2.replace(/nlemaccountmanager/gi, salesRepName);
				var emailAttach2 = new Object();
				emailAttach2["entity"] = custInternalID;
				...
				nlapiLogExecution(
					"DEBUG",
					"custInternalID after status to signed",
					custInternalID
				);

				var sales_rep_email_body = "New Customer Signed Up</br></br>";
				...
				sales_rep_email_body +=
					...
					"</br>";
				sales_rep_email_body +=
					"Customer Name: " + custEntityID + " " + custName + "</br>";
				sales_rep_email_body += "Franchisee: " + partner_text + "</br></br>";

				var sales_rep_cust_id_link =
				...
				sales_rep_email_body +=
					"<b><u>Customer Link</u></b>: " +
					sales_rep_cust_id_link +
					"</br></br> <b><u>New Auto Signed Up Customer List</u></b>: https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1657&deploy=1&compid=1048144";

				var sales_rep_email_subject =
					"Action Required: Activation Kit QR Code Scanned - " +
					custEntityID +
					" " +
					custName;
				...
				nlapiSendEmail(
					112209,
					["fiona.harrison@mailplus.com.au", "popie.popie@mailplus.com.au"],
					custEntityID +
						" " +
						custName +
						" - " +
						"Customer Account Created - Please Check & Finalise",
					email_body_internal,
					["ankith.ravindran@mailplus.com.au"],
					null,
					records,
				...
				intitial_customer_status == 71 ||
				intitial_customer_status == 66
			) {
				...
				var sales_rep_email_subject =
					"Action required: New Contact Added - " +
					custEntityID +
					" " +
					custName;
			...
			nlapiSendEmail(
				...
				email_subject,
				email_body,
				["mj@roundtableapps.com", "ankith.ravindran@mailplus.com.au"],
				null,
				records,
				null,
				true
			...
			nlapiRequestURL(
				"https://mpns.protechly.com/new_staff",
				userJSON,
				headers
			);
