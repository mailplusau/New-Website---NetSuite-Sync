	...
	for (var fieldname in data) {
		if (data.hasOwnProperty(fieldname)) {
			for (var x = 0; x < data[fieldname].length; x++) {
				nlapiLogExecution(
					"DEBUG",
					"data[fieldname][x][businessName]",
					data[fieldname][x]["businessName"]
				);
				nlapiLogExecution(
					"DEBUG",
					"data[fieldname][x][city]",
					data[fieldname][x]["city"]
				);
				nlapiLogExecution(
					"DEBUG",
					"data[fieldname][x][state]",
					data[fieldname][x]["state"]
				);
				nlapiLogExecution(
					"DEBUG",
					"data[fieldname][x][zip]",
					data[fieldname][x]["zip"]
				);
				nlapiLogExecution(
					"DEBUG",
					"data[fieldname][x][addr1]",
					data[fieldname][x]["addr1"]
				);
				nlapiLogExecution(
					"DEBUG",
					"data[fieldname][x][addr2]",
					data[fieldname][x]["addr2"]
				);
				if (
					isNullorEmpty(data[fieldname][x]["city"]) ||
					isNullorEmpty(data[fieldname][x]["state"]) ||
					isNullorEmpty(data[fieldname][x]["zip"]) ||
					data[fieldname][x]["city"] == "n/a" ||
					data[fieldname][x]["state"] == "n/a" ||
					data[fieldname][x]["zip"] == "n/a"
				) {
					dataOut +=
						'{"ns_id":"ADDRESS ERROR - Empty city, state and/or zip. The lead was not created."},';
				} else {
					//lead only created if the address is correct
					...
					var zeeNetworkMatrixSearchResults =
						zeeNetworkMatrixSearch.runSearch();
					...
					if (data[fieldname][x]["leadsource"].toLowerCase() == "portal") {
						...
						data[fieldname][x]["leadsource"].toLowerCase() == "shopify"
					) {
						customerRecord.setFieldValue("leadsource", 246306); //Shopify
					} else if (
						data[fieldname][x]["leadsource"].toLowerCase() == "airush"
					) {
					...
					customerRecord.setFieldValue(
						"custentity_frequency",
						data[fieldname][x]["frequency"]
					);
					...
					customerRecord.setCurrentLineItemValue(
						...
						"addr2",
						data[fieldname][x]["addr1"]
					); //reversed because comes the other way from the portal
					if (
						data[fieldname][x]["addr2"] != "n/a" &&
						data[fieldname][x]["addr2"] != "N/A"
					) {
						customerRecord.setCurrentLineItemValue(
							"addressbook",
					...
					customerRecord.setCurrentLineItemValue(
						"addressbook",
						"state",
						formatStateName(data[fieldname][x]["state"])
					);
					customerRecord.setCurrentLineItemValue(
						"addressbook",
						"zip",
					...
					var result = nlapiRequestURL(
						"https://maps.googleapis.com/maps/api/geocode/json?address=" +
							fullAddress +
							"&key=AIzaSyA92XGDo8rx11izPYT7z2L-YPMMJ6Ih1s0&libraries=places"
					);

					var resultJSON = JSON.parse(result.getBody());

					var subrecord = customerRecord.editCurrentLineItemSubrecord(
						"addressbook",
						"addressbookaddress"
					);
					...
					customerRecord.setFieldValue(
						"custentity_sendle_fuel_surcharge",
						"7.05"
					);
					...
					contactRecord.setFieldValue(
						"firstname",
						data[fieldname][x]["firstName"]
					);
					contactRecord.setFieldValue(
						"lastname",
						data[fieldname][x]["lastName"]
					);
					...
					//USER NOTE
					if (
						!isNullorEmpty(data[fieldname][x]["comments"]) ||
						data[fieldname][x]["comments"] != "N/A"
					) {
						...
						userNoteRecord.setFieldValue(
							"note",
							data[fieldname][x]["comments"]
						);
					...
					if (!isNullorEmpty(zee_id) && zeeCount == 1) {
						...
						customer_record.setFieldValue(
							"custentity_date_lead_quote_sent",
							getDate()
						);
					...
					if (isNullorEmpty(zee_id) || zeeCount > 1) {
						...
						var cc = [
							...
							"belinda.urbani@mailplus.com.au",
                    ...
                    */
					...
					var prodPricingRecord = nlapiCreateRecord(
                    ...
                    */
					...
					var exp500gItemPricingSearchResult =
						exp500gItemPricingSearch.runSearch();
					...
					var exp1kgItemPricingSearchResult =
						exp1kgItemPricingSearch.runSearch();
					...
					var exp3kgItemPricingSearchResult =
						exp3kgItemPricingSearch.runSearch();
					...
					var exp5kgItemPricingSearchResult =
						exp5kgItemPricingSearch.runSearch();
