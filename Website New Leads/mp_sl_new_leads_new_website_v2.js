...
function leadForm(request, response) {
  if (request.getMethod() == "GET") {
    ...
    nlapiLogExecution('DEBUG', 'how_did_you_hear_about_us',
      how_did_you_hear_about_us);
    ...
    if (isNullorEmpty(postcode)) {
      ...
      customerRecord.setCurrentLineItemValue('addressbook', 'addressee',
        business_name);
      customerRecord.setCurrentLineItemValue('addressbook', 'city', ' ');
      customerRecord.setCurrentLineItemValue('addressbook', 'state',
        formatStateName(postcode));
      ...
      if (splitPageURL[1] == 'api-integration-3pl/' || services_of_interest == '4') {
        ...
        body =
          ...
          entity_id + ' ' + customer_name + '\nLink: ' + cust_id_link;
        subject = 'API Sales HOT Lead - ' + entity_id + ' ' + customer_name +
          '';
      ...
      url += template_id + '&recid=' + customerRecordId + '&salesrep=' +
        salesRep + '&dear=' + full_name + '&contactid=' + null + '&userid=' +
        encodeURIComponent(nlapiGetContext().getUser());;
      urlCall = nlapiRequestURL(url);
      var emailHtml = urlCall.getBody();

      nlapiSendEmail(112209, email_address, templateSubject, emailHtml, null,
        null, emailAttach)
    }
