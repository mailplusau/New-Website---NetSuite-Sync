...
function leadForm(request, response) {
  if (request.getMethod() == "GET") {
    ...
    var current_carrier = request.getParameter("current_carrier");
    ...
    if (services_of_interest == "7") {
      ...
      service_of_interest_text =
        "Parcel & satchel delivery | Australia Post collect & lodge service";
    }
    ...
    // 	} else {
    // 		how_did_you_hear_about_us_text = "Social Media";
    // 	}
    ...
    var zeeNetworkMatrixSearch = nlapiLoadSearch(
      "partner",
      "customsearch_networkmtrx_zee_suburb_2"
    );
    ...
    var suburb_status = "";
    if (isNullorEmpty(zee_id) || zeeCount == 0) {
      zee_id = 435;
      zee_name = "MailPlus Pty Ltd";
      suburb_status = "OOT";
    ...
    //   scriptId: "customscript_sl2_pp_create_lead",
    ...
    var suiteletUrl = nlapiResolveURL(
      "SUITELET",
      "customscript_sl2_pp_create_lead",
      "customdeploy1",
      true
    );

    //Sample
    //{"custentity_primary_contact_phone":"0402712233","franchisee_name":"Rouse Hill","custentity_primary_contact_email":"ankith88@gmail.com","billzip":"2155","deploy":"1","billcountry":"Australia","custentity_abn":"","compid":"1048144","custentity_addr_lat":"-33.698885","custentity_primary_contact_name":"ANKITH RAVINDRAN","email":"ankith88@gmail.com","website":"","custentity_addr_long":"150.958347","script":"2194","custentity_checkin_questions":"Interested In: express\nWeekly Parcels: 1-10","billstate":"New South Wales","bucket":"inbound","custentity_primary_contact_title":"","franchisee_id":"632438","phone":"0402712233","companyname":"Test New Website 20260618 V2","ns-at":"AAEJ7tMQ6MIVXCrzpiLKSEmYLRVtAlRSAOWEC4Dyr1D-_83sS4g","custentity_leadsource":"Inbound","billaddr1":"10 Hezlett Road","category":"","billcity":"North Kellyville"}

    //Pass params via URL based on the above sample JSON
    suiteletUrl +=
      "&custentity_primary_contact_phone=" +
      phone_number +
      "&franchisee_name=" +
      zee_name +
      "&custentity_primary_contact_email=" +
      email +
      "&billzip=" +
      postcode +
      "&billcountry=" +
      ...
      "&custentity_addr_lat=" +
      lat +
      "&custentity_primary_contact_name=" +
      first_name +
      " " +
      last_name +
      "&email=" +
      ...
      "&website=" +
      "" +
      "&custentity_addr_long=" +
      lng +
      "&custentity_checkin_questions=Interested In: " +
      service_of_interest_text +
      " | Weekly Parcels: " +
      avg_daily_shipments_text +
      " | Current Carrier: " +
      current_carrier_text +
      " | LPO Notes: " +
      lpo_notes +
      "&weekly_parcels=" +
      avg_daily_shipments_text +
      ...
      "&bucket=" +
      "inbound" +
      "&custentity_primary_contact_title=" +
      "" +
      "&franchisee_id=" +
      zee_id +
      "&phone=" +
    ...
    // 2. Set any HTTP headers or parameters as needed
    var headers = new Array();
    headers["Content-Type"] = "application/json";

    // 3. Make the call (nlapiRequestURL automatically defaults to GET if no body is passed, POST if there is a body)
    var responseSuiteletURL = nlapiRequestURL(suiteletUrl, null, headers);

    // 4. Retrieve the response body
    var responseBody = responseSuiteletURL.getBody();
    ...
    // 		var resultSetlpoProfileLeadsListSearch = nlapiLoadSearch(
    // 			"customrecord_lpo_lead_form",
    // 			"customsearch_lpo_lead_profiles_list"
    // 		);

    // 		var newFiltersLPOProfileLead = new Array();
    // 		newFiltersLPOProfileLead[newFiltersLPOProfileLead.length] =
    // 			new nlobjSearchFilter(
    // 				"custrecord_lpo_lead_customer",
    // 				null,
    // 				"is",
    // 				parent_lpo
    // 			);
    ...
    // 		var resultSetlpoProfileLeadsListSearch =
    // 			resultSetlpoProfileLeadsListSearch.runSearch();
    // 		resultSetlpoProfileLeadsListSearch.forEachResult(function (
    // 			lpoProfileLeadsListSearchResultSet
    // 		) {
    // 			var lpoLeadProfileParentCustomer =
    // 				lpoProfileLeadsListSearchResultSet.getValue(
    // 					"custrecord_lpo_lead_customer"
    // 				);
    // 			lpoLeadProfileSalesRep = lpoProfileLeadsListSearchResultSet.getValue(
    // 				"custrecord_lpo_sales_rep"
    // 			);
    ...
    // 			customerRecord.setFieldValue(
    // 				"custentity_date_lpo_validated",
    // 				getDate()
    // 			); //LPO Account Status: Inactive
    ...
    // 			newFiltersLPOProfileLead[newFiltersLPOProfileLead.length] =
    // 				new nlobjSearchFilter(
    // 					"custentity_lpo_linked_franchisees",
    // 					"custrecord_lpo_lead_customer",
    // 					"anyof",
    // 					zee_id
    // 				);
    // 			resultSetlpoProfileLeadsListSearch.addFilters(
    // 				newFiltersLPOProfileLead
    // 			);
    ...
    // 				lpoProfileLeadsListSearchResultSet
    // 			) {
    // 				lpoLeadProfileParentCustomer =
    // 					lpoProfileLeadsListSearchResultSet.getValue(
    // 						"custrecord_lpo_lead_customer"
    // 					);
    // 				lpoLeadProfileSalesRep =
    // 					lpoProfileLeadsListSearchResultSet.getValue(
    // 						"custrecord_lpo_sales_rep"
    // 					);
    ...
    // 			);
    // 			mpStdActivated = partnerRecord.getFieldValue(
    // 				"custentity_zee_mp_std_activated"
    // 			);
    // 			mpExpActivated = partnerRecord.getFieldValue(
    // 				"custentity_zee_mp_exp_activated"
    // 			);
    // 			mpPrmActivated = partnerRecord.getFieldValue(
    // 				"custentity_zee_mp_str_activated"
    // 			);
    // 			zeeSalesRepAssigned = partnerRecord.getFieldValue(
    // 				"custentity_sales_rep_assigned"
    // 			);
    // 		}

    // 		nlapiLogExecution("DEBUG", "mpStdActivated", mpStdActivated);
    // 		nlapiLogExecution("DEBUG", "mpExpActivated", mpExpActivated);
    // 		nlapiLogExecution("DEBUG", "mpPrmActivated", mpPrmActivated);
    ...
    // 	customerRecord.setFieldValue(
    // 		"custentity_startrack_fuel_surcharge",
    // 		"24.85"
    // 	);
    ...
    // 	customerRecord.setCurrentLineItemValue(
    // 		"addressbook",
    // 		"label",
    // 		"Site Address"
    // 	);
    ...
    // 	var subrecord = customer_record.editCurrentLineItemSubrecord(
    // 		"addressbook",
    // 		"addressbookaddress"
    // 	);
    ...
    // 			) {
    // 				to = [
    // 					"paul.mcintosh@mailplus.com.au",
    // 					"alison.savona@mailplus.com.au",
    // 				];
    // 				body =
    ...
    // 						"\nLink: " +
    // 						cust_id_link;
    // 				} else if (state == "ACT" || state == "SA") {
    // 					var salesRep = 1862659; //Leonie Feata
    // 					to = ["leonie.feata@mailplus.com.au"];
    // 					body =
    // 						"Hi Leonie, \n \nA HOT Lead has been entered into the System.\n Customer Name: " +
    // 						entity_id +
    // 						" " +
    ...
    // 						"\nLink: " +
    // 						cust_id_link;
    // 				} else if (state == "QLD" || state == "WA" || state == "NT") {
    ...
    // 			} else {
    // 				var salesRecord = nlapiCreateRecord("customrecord_sales");
    // 				var salesRep = zeeSalesRepAssigned;
    // 				var to = zeeSalesRepAssigned;
    ...
    // 	//
    // 	var sendleZoneIDSearch = nlapiLoadSearch(
    // 		"customrecord_dom_zones",
    // 		"customsearch_sendle_dom_zones"
    // 	);

    // 	var newFiltersZoneID = new Array();
    // 	newFiltersZoneID[newFiltersZoneID.length] = new nlobjSearchFilter(
    // 		"custrecord_dom_zones_postcode",
    // 		null,
    // 		"is",
    // 		postcode.toString()
    // 	);
    // 	newFiltersZoneID[newFiltersZoneID.length] = new nlobjSearchFilter(
    // 		"custrecord_dom_zones_suburb_name",
    // 		null,
    // 		"is",
    // 		city
    // 	);
    ...
    // 	nlapiLogExecution(
    // 		"DEBUG",
    // 		"sendleZoneIDSearchResult.length",
    // 		sendleZoneIDSearchResult.length
    // 	);

    // 	var nsZoneID = 4;
    // 	sendleZoneIDSearchResult.forEachResult(function (
    // 		sendleZoneIDSearchResultSet
    // 	) {
    // 		nsZoneID = sendleZoneIDSearchResultSet.getValue(
    // 			"custrecord_dom_zones_ns_zones"
    // 		);
    ...
    // 	var std250gGoldItemPricingSearchResult =
    // 		std250gGoldItemPricingSearch.runSearch();

    // 	nlapiLogExecution(
    // 		"DEBUG",
    // 		"std250gGoldItemPricingSearchResult",
    // 		std250gGoldItemPricingSearchResult
    // 	);

    // 	var itemInternalstd250gID = null;
    // 	std250gGoldItemPricingSearchResult.forEachResult(function (
    // 		std250gGoldItemPricingSearchResultSet
    // 	) {
    // 		itemInternalstd250gID =
    // 			std250gGoldItemPricingSearchResultSet.getValue("internalid");

    // 		return true;
    // 	});

    // 	var std500gGoldItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var std500gGoldItemPricingSearchResult =
    // 		std500gGoldItemPricingSearch.runSearch();

    // 	var itemInternalstd500gID = null;
    // 	std500gGoldItemPricingSearchResult.forEachResult(function (
    // 		std500gGoldItemPricingSearchResultSet
    // 	) {
    // 		itemInternalstd500gID =
    // 			std500gGoldItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var std1kgGoldItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var std1kgGoldItemPricingSearchResult =
    // 		std1kgGoldItemPricingSearch.runSearch();

    // 	var itemInternalstd1kgID = null;
    // 	std1kgGoldItemPricingSearchResult.forEachResult(function (
    // 		std1kgGoldItemPricingSearchResultSet
    // 	) {
    // 		itemInternalstd1kgID =
    // 			std1kgGoldItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var std3kgGoldItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var std3kgGoldItemPricingSearchResult =
    // 		std3kgGoldItemPricingSearch.runSearch();

    // 	var itemInternalstd3kgID = null;
    // 	std3kgGoldItemPricingSearchResult.forEachResult(function (
    // 		std3kgGoldItemPricingSearchResultSet
    // 	) {
    // 		itemInternalstd3kgID =
    // 			std3kgGoldItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var std5kgGoldItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var std5kgGoldItemPricingSearchResult =
    // 		std5kgGoldItemPricingSearch.runSearch();

    // 	var itemInternalstd5kgID = null;
    // 	std5kgGoldItemPricingSearchResult.forEachResult(function (
    // 		std5kgGoldItemPricingSearchResultSet
    // 	) {
    // 		itemInternalstd5kgID =
    // 			std5kgGoldItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var std10kgGoldItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var std10kgGoldItemPricingSearchResult =
    // 		std10kgGoldItemPricingSearch.runSearch();

    // 	var itemInternalstd10kgID = null;
    // 	std10kgGoldItemPricingSearchResult.forEachResult(function (
    // 		std10kgGoldItemPricingSearchResultSet
    // 	) {
    // 		itemInternalstd10kgID =
    // 			std10kgGoldItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var std20kgGoldItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var std20kgGoldItemPricingSearchResult =
    // 		std20kgGoldItemPricingSearch.runSearch();

    // 	var itemInternalstd20kgID = null;
    // 	std20kgGoldItemPricingSearchResult.forEachResult(function (
    // 		std20kgGoldItemPricingSearchResultSet
    // 	) {
    // 		itemInternalstd20kgID =
    // 			std20kgGoldItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var std25kgGoldItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var std25kgGoldItemPricingSearchResult =
    // 		std25kgGoldItemPricingSearch.runSearch();

    // 	var itemInternalstd25kgID = null;
    // 	std25kgGoldItemPricingSearchResult.forEachResult(function (
    // 		std25kgGoldItemPricingSearchResultSet
    // 	) {
    // 		itemInternalstd25kgID =
    // 			std25kgGoldItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});
    ...
    // 	var premium10kgItemPricingSearchResult =
    // 		premium10kgItemPricingSearch.runSearch();

    // 	var itemInternalpremium10kgID = null;
    // 	premium10kgItemPricingSearchResult.forEachResult(function (
    // 		premium10kgItemPricingSearchResultSet
    // 	) {
    // 		itemInternalpremium10kgID =
    // 			premium10kgItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var premium20kgItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var premium20kgItemPricingSearchResult =
    // 		premium20kgItemPricingSearch.runSearch();

    // 	var itemInternalPremium20kgID = null;
    // 	premium20kgItemPricingSearchResult.forEachResult(function (
    // 		premium20kgItemPricingSearchResultSet
    // 	) {
    // 		itemInternalPremium20kgID =
    // 			premium20kgItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var premium1kgItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var premium1kgItemPricingSearchResult =
    // 		premium1kgItemPricingSearch.runSearch();

    // 	var itemInternalPremium1kgID = null;
    // 	premium1kgItemPricingSearchResult.forEachResult(function (
    // 		premium1kgItemPricingSearchResultSet
    // 	) {
    // 		itemInternalPremium1kgID =
    // 			premium1kgItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var premium3kgItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var premium3kgItemPricingSearchResult =
    // 		premium3kgItemPricingSearch.runSearch();

    // 	var itemInternalPremium3kgID = null;
    // 	premium3kgItemPricingSearchResult.forEachResult(function (
    // 		premium3kgItemPricingSearchResultSet
    // 	) {
    // 		itemInternalPremium3kgID =
    // 			premium3kgItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});

    // 	var premium5kgItemPricingSearch = nlapiLoadSearch(
    // 		"noninventoryitem",
    // 		"customsearch3745"
    // 	);
    ...
    // 	var premium5kgItemPricingSearchResult =
    // 		premium5kgItemPricingSearch.runSearch();

    // 	var itemInternalPremium5kgID = null;
    // 	premium5kgItemPricingSearchResult.forEachResult(function (
    // 		premium5kgItemPricingSearchResultSet
    // 	) {
    // 		itemInternalPremium5kgID =
    // 			premium5kgItemPricingSearchResultSet.getValue("internalid");
    // 		return true;
    // 	});
    ...
    // 				"custentity_zee_mp_std_activated"
    // 			);
    // 			var mp_exp_activated = partner_record.getFieldValue(
    // 				"custentity_zee_mp_std_activated"
    // 			);
    // 			if (
    // 				(mp_std_activated != 1 || mp_std_activated != "1") &&
    // 				(mp_exp_activated == 2 || mp_exp_activated == "2")
    // 			) {
    ...
    // 					templateSubject,
    // 					emailHtml,
    // 					["luke.forbes@mailplus.com.au", "lee.russell@mailplus.com.au"],
    // 					null,
    // 					emailAttach,
    // 					emailAttach
    // 				);
...
function getTerritory(lat, lng) {
  ...
  for (k = 0; k < territories.length; k++) {
    ...
    nlapiLogExecution(
      "DEBUG",
      "polygon" + territories[k].properties.Territory + "",
      polygon
    );
    nlapiLogExecution(
      "DEBUG",
      "polygon.length" + territories[k].properties.Territory + "",
      polygon.length
    );
    var isInTerritory = inside([lng, lat], polygon);
    nlapiLogExecution(
      "DEBUG",
      "isInTerritory" + territories[k].properties.Territory + "",
      isInTerritory
    );
    if (isInTerritory == true) {
      ...
      nlapiLogExecution(
        "DEBUG",
        "territories[k].properties.Territory",
        territories[k].properties.Territory
      );
